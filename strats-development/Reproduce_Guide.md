...
[cite_start]This guide provides a complete, step-by-step pipeline to reproduce all experiments, including preprocessing, training, and evaluation[cite: 1].

---

## [cite_start]1. Environment Setup 
...
for p in "${PERTS[@]}"; do 
  for s in "${SEEDS[@]}"; do [cite: 3]
    for pct in "${PCTS[@]}"; do 
      python3 src/preprocess_mimic_iii_${p}.py --data_dir data/processed --out_dir data/processed --seed $s --pct $pct 
      python3 src/preprocess_physionet_2012_${p}.py --data_dir data/processed --out_dir data/processed --seed $s --pct $pct 
    done 
  done 
done 
Unbalanced (no seed) 
...
Bash
for pct in "${PCTS[@]}"; do 
  python3 src/preprocess_mimic_iii_unbalanced.py --data_dir data/processed --out_dir data/processed --pct $pct 
  python3 src/preprocess_physionet_2012_unbalanced.py --data_dir data/processed --out_dir data/processed --pct $pct 
done 
4. Training 
