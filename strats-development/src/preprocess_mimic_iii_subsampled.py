...
# --------------------------------------------------------
parser = argparse.ArgumentParser(description="Create MIMIC-III subsets (downsampling train+val) for STRaTS")
parser.add_argument("--data_dir", type=str, required=True, help="Path to MIMIC-III extracted/processed folder")
...
print(f"\n➡ Creating mimic_{pct}.pkl | keeping {len(keep_ids)}/{len(train_ids) + len(val_ids)} train+val")
...
out_path = os.path.join(OUT_DIR, f"mimic_iii_subsampled_{pct}_{args.seed}.pkl")
